...
% pplane8b  is an interactive tool for studying planar autonomous systems of
%	differential equations.  When pplane8b is executed, a pplane8b Setup
...
%
%	When the Proceed button is pressed on the Setup window, the pplane8b
...
%	the pplane8b Display window, the solution to the system with that
...
elseif strcmp(action,'plotxyfig')  % Build the graph figure.
  ...
  ppxy = figure('number','off',...
		'tag','pplane8b','visible','off');
  name = ['pplane8b t-plot # ',int2str(ppxy)];
  ud.color = dud.color;
  ud.type = 0;
